...
author:
  - name: "Philippe MICHEL"
    affiliations: "USRC - Hôpital NOVO"
date: last-modified
project:
  type: website
  output-dir: "docs"
...
# De quoi on parle ? {background-color="lightblue"}
...
# Que lire ? {background-color="lightblue"}
...
**Comparison of early versus late initiation of renal replacement
therapy in critically ill patients with acute kidney injury: a
systematic review and meta-analysis.**\n Critical Care 2011, 15:R72

 

**Conclusion** Earlier institution of RRT in critically ill patients
with AKI may have a beneficial impact on survival. However, this
conclusion is based on heterogeneous studies of variable quality and
only two randomized trials. In the absence of new evidence from
suitably-designed randomized trials, a definitive treatment
...
::: {.column width="39%"}
<BR> ![](images/experts.png){.r-stretch}
:::
:::::

## Où chercher ?

![](images/pubmed.jpg){.absolute top="180" right="30" width="80%"}
...
##  {background-color="grey10" background-image="images/pubmed2.png" background-size="100%"}

##  {background-color="grey10" background-image="images/pubmed3.png" background-size="100%"}

##  {background-color="grey10" background-image="images/google.png" background-size="100%"}

##  {background-color="grey10" background-image="images/zotero.png" background-size="100%"}

##  {background-color="grey10" background-image="images/scihub.png" background-size="100%"}
...
# Quelle étude ? {background-color="lightblue"}
...
::::: columns
::: {.column width="40%"}
> **Comité d'Éthique**<BR> **CNIL**<BR> **CPP**
:::
...
# On rédige {background-color="lightblue"}
...
# Matériel & Méthodes {background-color="lightblue"}
...
**Patient Selection** Twenty-two patients with ARDS were recruited from
the intensive care units (ICUs) of the Di Venere, Policlinico
(University of Bari), and S. Chiara (University of Pisa) hospitals. The
review boards of ;all three hospitals approved the research protocol,
and informed consent was obtained from all patients or next of kin.
Inclusion criteria were age \>18 yr and diagnosis of ARDS. Exclusion
criteria were cardiogenic pulmonary edema (clinically suspected or
pulmonary artery occlusion pressure \> 18 mmHg), history of ventricular
fibrillation or tachyarrhythmia, unstable angina or myocardial
infarction within the preceding month, preexisting chronic obstructive
pulmonary disease, mean arterial pressure (MAP) \< 65 mmHg (despite
attempts to increase blood pressure with fluid and vasopressors, as
clinically indicated), anatomic chest wall abnormalities, chest tube
with persistent air leak, pregnancy, and intracranial
abnormality.<BR>All patients were intubated and ventilated with a
Siemens Servo Ventilator 300 (Siemens Elema AB, Solna, Sweden). The
investigation was performed in the semirecumbent position after sedation
(diazepam, 0.1–0.2mg/kg, and fentanyl, 2–3{ng/Kg) and paralysis
(vecuronium, 4–8 mg). A physician not involved in the research aspects
of the study was always present to provide patient care.
...
Aspirine & reperfusion dans l'IdM en phase aiguë<br> Lancet 1986
...
-   Vérifier si le traitement proposé est conforme à ce que vous
    attendiez, proche de votre pratique…
...
**Statistics** Data are expressed as mean ± standard deviation of the
mean. Data within groups were compared by analysis of variance for
repeated measures with a *Bonferroni* correction. If significant (p \<
0.05), the values at different experimental conditions were compared
with those at baseline with use of a paired t test, as modified by
Dunnett. Comparisons of data between groups were performed at each
experimental condition by the Fisher exact test for categorical
variables and the Wilcoxon test for continuous variables. Regression
analysis was performed with the least-squares method. Analysis was
carried out with the StatView software package (Abacus, Berkeley, CA).
...
Doit mettre en évidence les conclusions pratiques de l'étude mais aussi
...
-   Mais elle a ses limites :
    -   Patients très âgés;
    -   …
-   Les autres études montrent que …

## Conclusion

::::: columns
::: {.column width="48%"}
<BR> - Résumé de la discussion<BR> - Fin ouverte
:::
...
# Des variables ? {background-color="lightblue"}
...
::: {.column width="48%"}
<p><font size="7">Âge <input type ='NUMBER' 
  name = 'age' 
  min = '18' 
  max = '110'></font></p>
:::
...
# Organiser son tableau {background-color="lightblue"}
...
# Que faire des données ? {background-color="lightblue"}
...
##  {background-color="grey10" background-image="images/data5.png" background-size="60%"}

##  {background-color="grey10" background-image="images/carte.jpg" background-size="80%"}

##  {background-color="grey10" background-image="images/echo.jpg" background-size="100%"}
...
##  {background-color="grey10" background-image="images/bio1.png" background-size="90%"}

##  {background-color="grey10" background-image="images/bio2.png" background-size="90%"}
...
##  {background-color="grey10" background-image="images/multi.png" background-size="80%"}

## Un peu de recul…

$$IC_{95}= p \pm z_{\alpha/2}\sqrt{\frac{p(1-p)}{n}}$$

<br> $$\mathbb{P}\left(-z_{\alpha/2} \leq
...
(images/pavlue1.png){.center}

## p\< 5  ???

![](images/pavlue2.png){.center}

## p\< 5  ???

![](images/pavlue3.png){.center}

## CONCLUSION 1

![](images/garbage.jpg){.center}
